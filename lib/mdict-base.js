...
var _wordBuffer = require("./wordBuffer");
...
var MDictBase = /*#__PURE__*/function () {
  ...
  function MDictBase(fname, passcode) {
    var _config$keyCaseSensit, _config$stripKey;

    var config = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    (0, _classCallCheck2["default"])(this, MDictBase);
    // the mdict file name
    this.fname = fname; // the dictionary file decrypt pass code

    this._passcode = passcode; // the mdict file read offset

    this._offset = 0; // the dictionary file extension
    ...
    this._whetherKeyCaseSensitive = (_config$keyCaseSensit = config.keyCaseSensitive) !== null && _config$keyCaseSensit !== void 0 ? _config$keyCaseSensit : _common["default"].isTrue(this.header.keyCaseSensitive);
    this._whetherStripKey = (_config$stripKey = config.stripKey) !== null && _config$stripKey !== void 0 ? _config$stripKey : _common["default"].isTrue(this.header.StripKey); // -------------------------
    ...
    this._keyBlockInfoEndOffset = 0; // key block info list
    ...
    if (this.ext === 'mdx') {
      _utils["default"].consoleMem('pre buffer: ');

      this.keyList = []; // decodeKeyBlock method is very slow, avoid invoke dirctly
      ...
      _utils["default"].measureTime('decodeKeyBlock', this._decodeKeyBlock.bind(this), true); // this._decodeKeyBlock(true);


      _utils["default"].measureTime('_keyToBuffer', this._keyToBuffer.bind(this));

      _utils["default"].consoleMem('after buffer: ');
    ...
    this.keyData = []; // decodeRecordBlock method is very slow, avoid invoke dirctly
  ...
  (0, _createClass2["default"])(MDictBase, [{
    ...
    value: function _readHeader() {
      ...
      var headerByteSize = _common["default"].readNumber(header_size_buffer, _common["default"].NUMFMT_UINT32); // [4:header_byte_size + 4] header_bytes


      var headerBuffer = _readChunk["default"].sync(this.fname, 4, headerByteSize); // TODO: SKIP 4 bytes alder32 checksum
      ...
      this._keyHeaderStartOffset = headerByteSize + 4 + 4; // set file read offset

      this._offset = this._headerEndOffset; // header text in utf-16 encoding ending with `\x00\x00`, so minus 2

      var headerText = _common["default"].readUTF16(headerBuffer, 0, headerByteSize - 2); // parse header info
    ...
    value: function _readKeyHeader() {
      ...
      // set offset
      this._keyHeaderStartOffset = this._headerEndOffset; // version >= 2.0, key_header bytes number is 5 * 8, otherwise, 4 * 4
      ...
      var ofset = 0; // [0:8]   - number of key blocks
      ...
      ofset += this._numWidth; // console.log("num_key_blocks", num_key_blocks.toString());
      ...
      ofset += this._numWidth; // console.log("num_entries", num_entries.toString());
      ...
      if (this._version >= 2.0) {
        ...
        ofset += this._numWidth; // console.log(key_block_info_decomp_size.toString());
      ...
      ofset += this._numWidth; // console.log("key_block_info_size", key_block_info_size.toString());

      this.keyHeader.keyBlockInfoCompSize = keyBlockInfoSize; // [32:40] - number of key blocks total size, note, key blocks total size, not key block info
      ...
      ofset += this._numWidth; // console.log(key_blocks_total_size.toString());

      this.keyHeader.keyBlocksTotalSize = keyBlocksTotalSize; // 4 bytes alder32 checksum, after key info block
      // TODO: skip for now, not support yet

      if (this._version >= 2.0) {// this.__skip_bytes(4);
      } // set end offset


      this._keyHeaderEndOffset = this._keyHeaderStartOffset + bytesNum + (this._version >= 2.0 ? 4 : 0);
      /* 4 bytes adler32 checksum length, only for version >= 2.0 */
    }
    /**
    ...
    value: function _readKeyBlockInfo() {
      ...
      this.keyBlockInfoList = keyBlockInfoList; // NOTE: must set at here, otherwise, if we haven't invoke the _decodeKeyBlockInfo method,
    ...
    value: function _decodeKeyBlockInfo(keyBlockInfoBuff) {
      ...
      if (this._version >= 2.0) {
        ...
        kbInfoBuff = _pako["default"].inflate(kbInfoCompBuff.slice(8, kbInfoCompBuff.length)); // TODO: check the alder32 checksum
        // adler32 = unpack('>I', key_block_info_compressed[4:8])[0]
        // assert(adler32 == zlib.adler32(key_block_info) & 0xffffffff)
        // this.keyHeader.keyBlockInfoDecompSize only exist when version >= 2.0

        (0, _assert["default"])(this.keyHeader.keyBlockInfoDecompSize == kbInfoBuff.length, 'key_block_info length should equal');
      } else {
        kbInfoBuff = keyBlockInfoBuff;
      }

      var key_block_info_list = []; // init tmp variables
      ...
      while (kbCount < keyBlockNum) {
        ...
        i += byteWidth; // step gap

        var stepGap = 0; // term_size is for first key and last key
        // let term_size = 0;
        ...
        if (this._encoding === UTF16 || this.ext === 'mdd') {
          stepGap = (lastKeySize + textTerm) * 2; // TODO: this is for last key output
    ...
    value: function _reduceWordKeyBlock(phrase, _s, compareFn) {
      ...
      var mid = 0; // when compare the word, the uppercase words are less than lowercase words
    ...
    key: "_decodeKeyBlock",
    value: function _decodeKeyBlock(keep) {
      ...
      var key_list = [];
      var keyCount = 0;
      var kbStartOfset = 0; // harvest keyblocks
      ...
      for (var idx = 0; idx < this.keyBlockInfoList.length; idx++) {
        ...
        var end = kbStartOfset + compSize; // 4 bytes : compression type

        var kbCompType = new _bl["default"](kbCompBuff.slice(start, start + 4)); // TODO 4 bytes adler32 checksum
        ...
        if (kbCompType.toString('hex') == '00000000') {
          ...
          // decompress key block
          key_block = _pako["default"].inflate(kbCompBuff.slice(start + 8, end)); // extract one single key block into a key list
        ...
        var keyBlockRes = this._splitKeyBlock(new _bl["default"](key_block), keep);
      ...
      (0, _assert["default"])(keyCount === this.keyHeader.entriesNum);
      this._keyBlockEndOffset = this._keyBlockStartOffset + this.keyHeader.keyBlocksTotalSize;

      if (keep) {
        this.keyList = key_list; // console.log('keep: ', keep)
    ...
    value: function _decodeKeyBlockByKBID(kbid) {
      ...
      var splitedKey = this._splitKeyBlock(new _bl["default"](key_block), true);
    ...
    key: "_splitKeyBlock",
    value: function _splitKeyBlock(keyBlock, needList) {
      var delimiter;
      var width;
      ...
      while (keyStartIndex < keyBlock.length) {
        ...
        // 0.2656s
        var recordStartOffset = _common["default"].readNumber(keyBlock.slice(keyStartIndex, keyStartIndex + this._numWidth), this._numFmt); // 0.2746s
        ...
        while (i < keyBlock.length) {
          // delimiter = '0' 
          if (width === 1 && keyBlock.get(i) == 0 || // delimiter = '00'
        ...
        var keyText = this._decoder.decode(keyBlock.slice(keyStartIndex + this._numWidth, keyEndIndex)).trim();
    ...
    value: function _decodeRecordInfo() {
      ...
      this._recordInfoEndOffset = this._recordInfoStartOffset + this.recordHeader.recordBlockInfoCompSize; // avoid user not invoke the _decodeRecordBlock method
    ...
    value: function _decodeRecordBlock() {
      ...
      for (var idx = 0; idx < this.recordBlockInfoList.length; idx++) {
        ...
        recordOffset += compSize; // 4 bytes: compression type

        var rbCompType = new _bl["default"](rbCompBuff.slice(0, 4)); // record_block stores the final record data
        ...
        if (rbCompType.toString('hex') === '00000000') {
          ...
          // decrypt
          // --------------
          var blockBufDecrypted = null; // if encrypt type == 1, the record block was encrypted

          if (this._encrypt === 1
          /* || (this.ext == "mdd" && this._encrypt === 2 ) */
          ) {
          ...
          if (rbCompType.toString('hex') === '01000000') {
            comp_type = 'lzo'; // the header was need by lzo library, should append before real compressed data

            var header = new ArrayBuffer([0xf0, decompSize]); // Note: if use lzo, here will LZO_E_OUTPUT_RUNOVER, so ,use mini lzo js
            ...
            comp_type = 'zlib'; // zlib decompress

            recordBlock = _pako["default"].inflate(blockBufDecrypted);
          }
        }

        recordBlock = new _bl["default"](recordBlock); // notice that adler32 return signed value
        ...
        while (i < this.keyList.length) {
          ...
          var keyText = this.keyList[i].keyText; // # reach the end of current record block
          ...
          i += 1; // const data = record_block.slice(record_start - offset, record_end - offset);
    ...
    value: function _decodeRecordBlockByRBID(rbid, keyText, start, nextStart) {
      ...
      var rbCompBuff = this._readBuffer(startOffset, compSize); // 4 bytes: compression type


      var rbCompType = new _bl["default"](rbCompBuff.slice(0, 4)); // record_block stores the final record data

      var recordBlock; // TODO: igore adler32 offset
      // Note: here ignore the checksum part
      // bytes: adler32 checksum of decompressed record block
      // adler32 = unpack('>I', record_block_compressed[4:8])[0]
    ...
    key: "_keyToBuffer",
    value: function _keyToBuffer() {
      var _this = this;

      var ruleKeyFn = function () {
        var key = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';

        if (_this._whetherStripKey) {
          key = key.replace(_common["default"].REGEXP_STRIPKEY[_this.ext], '$1');
        }

        if (!_this._whetherKeyCaseSensitive) {
          key = key.toLowerCase();
        }
      ...
      this.keyBuffer = new _wordBuffer.WordBuffer(undefined, ruleKeyFn);
      var list = (0, _toConsumableArray2["default"])(this.keyList); // 排序之前记录下，每个单词的结束位置，因为排序之后顺序就乱了，buffer 里就不能再根据下一个单词判断了
      ...
      list.sort(function (a, b) {
        var aKey = ruleKeyFn(a.keyText);
        var bKey = ruleKeyFn(b.keyText);
