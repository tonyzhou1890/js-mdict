...
var Mdict = /*#__PURE__*/function (_MdictBase) {
  ...
  (0, _createClass2["default"])(Mdict, [{
    ...
    value: function _lookupMixed(word) {
      ...
      var recordList = [];
      var regexp = _common["default"].REGEXP_STRIPKEY[this.ext];
      var key = (word !== null && word !== void 0 ? word : '').replace(regexp, '$1').toLowerCase();
      recordList = (_this$trie$lookup = this.trie.lookup(key)) !== null && _this$trie$lookup !== void 0 ? _this$trie$lookup : []; // if not found the key block, return undefined
      ...
      for (var i = 0; i < recordList.length; i++) {
        var _recordList$i$endOffs;

        var rid = this._reduceRecordBlock(recordList[i].recordStartOffset);

        var nextStart = (_recordList$i$endOffs = recordList[i].endOffset) !== null && _recordList$i$endOffs !== void 0 ? _recordList$i$endOffs : this._recordBlockStartOffset + this.recordBlockInfoList[this.recordBlockInfoList.length - 1].decompAccumulator + this.recordBlockInfoList[this.recordBlockInfoList.length - 1].decompSize;
