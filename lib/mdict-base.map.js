...
var MDictBase = /*#__PURE__*/function () {
  ...
  function MDictBase(fname, passcode, mode) {
    ...
    if (this.ext === 'mdx' && this.mode === 'mixed') {
      _utils["default"].consoleMem('pre map');
      ...
      _utils["default"].consoleMem('after map');

      _utils["default"].measureTime('_buildKeyMap', this._buildKeyMap.bind(this), this.keyList);

      _utils["default"].consoleMem('after map'); // this._buildKeyMap(this.keyList)
  ...
  (0, _createClass2["default"])(MDictBase, [{
    ...
    value: function _decodeKeyBlock(keep) {
      ...
      for (var idx = 0; idx < this.keyBlockInfoList.length; idx++) {
        ...
        var splitedKey = this._splitKeyBlock(new _bl["default"](key_block), this._numFmt, this._numWidth, this._encoding);
    ...
    key: "_buildTrie",
    value: function _buildTrie(list) {
      var key = '';
    ...
    key: "_buildKeyMap",
    value: function _buildKeyMap(list) {
      var key = '';
      var regexp = _common["default"].REGEXP_STRIPKEY[this.ext];
      ...
      for (var i = 0; i < list.length; i++) {
        ...
        list[i] = new Uint32Array([list[i].recordStartOffset, list[i].endOffset].concat((0, _toConsumableArray2["default"])(_utils["default"].strToUint32(list[i].keyText))));
